...
O mecanismo de tratamento de exceção sempre foi presente na linguagem Java entre tando em Java 7 forneceu uma evolução elegante que foi a opção do desenvolvedor utilizar \texttt{multi-catch} que possibilita a concatenação de \texttt{catchs} iguais ou similares. Com isso a adoção de recurso permite a redução da lógica duplicada em \texttt{catchs} distintos de uma construção \texttt{try-catch}.
