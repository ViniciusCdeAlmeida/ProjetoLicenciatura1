...
escrito na linguagem Java, realiza o \textit{parse} desses arquivos gerando uma representação 
...
da representa\c c\~{a}o correspondem aos elementos sint\'{a}ticos da linguagem (como pacotes, classes, interfaces métodos e atributos). 
...
para travessia na AST, de acordo com o padr\~{a}o de projeto \textit{Visitors}~\cite{Gamma:1995}, e que facilita a análise estática 
de código fonte.
...
que visam obter maior facilidade e flexibilidade na exporta\c c\~{a}o das informa\c c\~{o}es coletadas durante a travessia nos n\'{o}s das 
...
um parser existente); processados e analisados com uma infraestrutura de \emph{visitors}, e os resultados das an\'{a}lises s\~{a}o por fim exportados.
...
onde o tipo \textbf{T} deve corresponder a classe usada para armazenar as informações coletadas pelo \emph{visitor}. O parâmetro de tipo \textbf{T} faz referência a uma 
...
Em geral, de acordo com a arquitetura do analisador est\'{a}tico proposto, para cada constru\c c\~{a}o que se 
...
A Tabela:~\ref{tab:VisitorsCriados} exibe todos os \textit{Visitors} criados neste projeto com sua respectiva descrição. A complexidade ciclomática dos \textit{Visitors} a qual é dada pela quantidade de caminhos independentes em um trecho de código, vale ressaltar que a complexidade dos \textit{Visitors} varia de 1 a 8 sendo a média 2,5 e a quantidade média de linha de código total necessária para escrever cada \textit{Visitor} é 47.


\begin{table}[ht!] \footnotesize
	\centering
	\caption{Visitors com a respectiva descrição, complexidade ciclomática e LOC.}
	\label{tab:VisitorsCriados}	
    \begin{tabular}{ >{\arraybackslash}p{1.7in} | >{\arraybackslash}m{3.2in} | >{\arraybackslash}p{0.3in} | >{\arraybackslash}p{0.3in}}
%		\begin{tabular}{M|p{9cm}}% centered column
			\hline 
			\textbf{Visitor} & \textbf{Atribuição} & \textbf{\acs{CC}} & \textbf{\acs{LOC}}\\ \hline \hline
			
			AIC & Pesquisar \textit{Anonymous Inner Class} declaradas. & 6 & 64\\ \hline
			
		
			ExistPattern & Pesquisa \textit{EnhancedFor} que iteram sobre uma coleção procurando qualquer ocorrência nessa coleção. & 28 & 116\\ \hline
			
			FieldAndVariableDeclaration & Lista todos as variáveis declaradas como os respectivos tipos. & 8 & 81\\ \hline
			
			FilterPattern &  Lista todos os \textit{EnhancedFor} que iteram uma coleção filtrando elementos desta mesma coleção. & 43 & 168\\ \hline
			
			ImportDeclaration & Lista todos os \textit{imports} existentes em cada classe. & 1 & 63\\ \hline
			
			LambdaExpression & Pesquisa casos de utilização da expressões lambda. & 1 & 33\\ \hline
			
			Lock & Verifica se nos métodos declarados existe alguma variável chamada Lock, ReentrantLock, ReadLock ou WriteLock.  & 12 & 75\\ \hline
			
			MapPattern & Pesquisa \textit{EnhancedFor} que iteram sobre uma coleção onde seja aplicado algum método sobre os itens desta coleção. & 27 & 103\\ \hline
			
			MethodCall & Verifica onde esta sendo utilizado reflection no projeto. & 2 & 22\\ \hline
			
			MethodDeclaration & Coleta informações sobre os métodos declarados nos projetos. & 5 & 45\\ \hline
			
			ScriptingEngine & Verifica se o projeto faz chamada a algum \textit{Scripting}. & 5 & 39\\ \hline
			
			SwitchStatement & Pesquisa \textit{Switchs} que utilizam \textit{String} como parâmetro. & 3 & 35\\ \hline
			
			SwitchStringOpportunities &  Pesquisa \textit{If-Else} aninhados onde no \textit{If} contenha \textit{String}, caracterizando uma possibilidade de adoção de \textit{Switch} com \textit{String}. & 3 & 50\\ \hline
		
			TryStatement & Pesquisa \textit{trys} que utilizar \textit{resource}, adoção de \textit{multicatch} e \textit{trys} que possuem \textit{catchs} aninhados. & 11 & 80\\ \hline
			
			TypeDeclaration & Pesquisa todos os tipos declarados. & 2 & 41\\ \hline 		
		\end{tabular}
\end{table}
