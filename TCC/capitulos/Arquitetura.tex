\chapter{Analisador Est\'{a}tico e Caracteriza\c c\~{a}o do Uso de Constru\c c\~{o}es da Linguagem Java}\label{cap:arquiteruta}
...
informa\c c\~{o}es. Utilizando esse suporte ferramental, 
descrito na Se\c c\~{a}o~\ref{sec:ferramenta}, foi conduzido 
um estudo emp\'{i}rico para caracterizar o uso de 
constru\c c\~{o}es da linguagem Java, cujos resultados 
est\~{a}o descritos na Se\c c\~{a}o~\ref{sec:resultados}.

\section{Analisador Est\'{a}tico}\label{sec:ferramenta}
...
para travessia na \acs{AST}, de acordo com o padr\~{a}o de projeto \textit{Visitor}~\cite{Gamma:1995}, e 
...
sem que a estrutura da \'{a}rvore de objetos sofra alterações, de modo que tornou-se relativamente  
...
pr\'{o}ximas se\c c\~{o}es 3.1.1, 3.1.2 e 3.1.3 apresentam mais detalhes sobre a arquitetura e 
...
\subsection{Entrada do Analisador Est\'{a}tico}
...
Os arquivos contendo código fonte Java servem ent\~{a}o como a entrada 
descrita na representa\c c\~{a}o abstrata 
do analisador est\'{a}tico como descrito na Figura~\ref{fig:Arquitetura}. Ou seja, para cada 
...
(ver Linha 25 no Listing:~\ref{lst:programaPrincipal}). Ap\'{o}s a lista de 
...
processar a base de c\'{o}digo fonte de cada projeto. A Figura~\ref{fig:metodoAnalyse} 
...
em um arquivo fonte produz uma inst\^{a}ncia da classe 
...
\subsection{Análise da Representa\c c\~{a}o Intermedi\'{a}ria}
...
\subsubsection{Descri\c c\~{a}o dos Visitors}
...
\subsubsection{Extensibilidade para Inclus\~{a}o de Novos Visitors}
...
\'{e} um mecanismo de extensibilidade mais 
conhecido como um padr\~{a}o de projeto originalmente denominado de inversão de controle (\emph{Inversion of Control}, \acs{IoC}). 
...
\acs{CSV}. Esse formato facilita as an\'{a}lises estat\'{i}sticas usando o ambiente e linguagem 
...
se\c c\~{a}o anterior, e introspec\c c\~{a}o de c\'{o}digo, via \acs{API} \textit{Reflection} da linguagem de 
programa\c c\~{a}o Java. Tal mecanismo oferece aos programadores a capacidade de escrever componentes 
